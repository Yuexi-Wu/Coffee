...
public class GourmetCoffee {
	...
	private Catalog loadCatalog() {
		...
		catalog.addItems(new Coffee("C001", "Colombia, Whole, 1 lb", 17.99, "Colombia", "Medium", "Rich and Hearty",
				"Rich", "Medium", "Full"));
		catalog.addItems(new Coffee("C002", "Colombia, Ground, 1 lb", 18.75, "Colombia", "Medium", "Rich and Hearty",
				"Rich", "Medium", "Full"));
		catalog.addItems(new Coffee("C003", "Italian Roasts, Whole, 1 lb", 16.80, "Latin American Blend",
				"Italian Roast", "Dark and heavy", "Intense", "Low", "Medium"));
		catalog.addItems(new Coffee("C004", "Italian Roasts, Ground, 1 lb", 17.55, "Latin American Blend",
				"Italian Roast", "Dark and heavy", "Intense", "Low", "Medium"));
		catalog.addItems(new Coffee("C005", "French Roasts, Whole, 1 lb", 16.80, "Latin American Blend",
				"French Roast", "Bittersweet, full intense", "Intense, full", "None", "Medium"));
		catalog.addItems(new CoffeeBrewer("B001", "Home Coffee Brewer", 150.00, "Brewer 100", "Pourover", 6));
		catalog.addItems(new CoffeeBrewer("B002", "Coffee Brewer, 2 Warmers", 200.00, "Brewer 200", "Pourover", 12));
		catalog.addItems(new CoffeeBrewer("B003", "Coffee Brewer, 3 Warmers", 280.00, "Brewer 210", "Pourover", 12));

		catalog.addItems(new Product("A001", "Almond Flavored Syrup", 9.00));
		catalog.addItems(new Product("A002", "Irish Creme Flavored Syrup", 9.00));
		catalog.addItems(new Product("A003", "Mint Flavored syrup", 9.00));
		catalog.addItems(new Product("A004", "Caramel Flavored Syrup", 9.00));
		catalog.addItems(new Product("A005", "Gourmet Coffee Cookies", 12.00));
	...
	public void displayOrder() {
		...
		if (size == 0) {
			...
			stdOut.println("Total: " + CURRENCY.format(this.currentOrder.getTotalOrderMoney()));
		}
	}

	/**
	 * Modifies the current order: if the specified product is not already part
	 * of the order, it is added; otherwise, the quantity of the specified
	 * product is updated.
	 */
	public void addModifyProduct() throws IOException {
		...
		if (orderItem == null) {
			...
			stdOut.println("The product " + product.getCode() + " has been added");
		} else {
	...
	public void displayOrdersSold() {
		...
		if (numOrders != 0) {
			...
			for (Order order : this.sales) {
				...
				stdOut.println("   Total: " + CURRENCY.format(order.getTotalOrderMoney()));
			}
		} else {
	 ...
	 * 
	 * @param product
	 *            the <code>Product</code> object to be displayed.
	 */
	public void displayNumberOfOrders(Product product) {
		...
		if (numOrders != 0) {
			...
			stdOut.println(" Number of orders that contains the product:" + product.getCode() + " Numbers:" + orderNumber);
		} else {
			stdErr.println("The order is empty.");
		}
	}
	...
	public void displayTotalQuantityOfProducts() {
		...
		if (numOrders != 0) {
			...
			stdErr.println("There are no products.");
		}
	...
	/*
	 * Prompts user for the product quantity and verifies the user's response.
	 * 
